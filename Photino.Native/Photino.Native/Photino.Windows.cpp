...
static std::wstring ComputeHostUrl(const std::wstring& appUrl);
...
void Photino::Register(HINSTANCE hInstance)
{
	...
	RegisterClassEx(&wcx);
	PhotinoSurface::RegisterWindowClass(hInstance);
...
Photino::Photino(PhotinoInitParams* initParams)
{
	...
	if (initParams->Size != PHOTINO_INIT_PARAMS_SIZE)
	{
		wchar_t msg[200];
		swprintf(msg, 200, L"Initial parameters passed are %i bytes, but expected %i bytes.", initParams->Size, PHOTINO_INIT_PARAMS_SIZE);
	...
	_compositionHosting = initParams->CompositionHosting;
	_transparentAtCreation = initParams->Transparent;
	_composition = nullptr;
	_suspendedResources = 0;
	_autoSuspendMask = (unsigned int)initParams->AutoSuspendOnMinimize & PHOTINO_RES_ALL;
	_autoSuspendedResources = 0;
	_pendingSuspendMask = 0;
	_suspendGeneration = 0;
	_audioWasMuted = false;
	...
	_hostHidden = false;
	if (_compositionHosting && _surfaceHostMode == PHOTINO_SURFACE_HOST_FRAMES && _startUrl != NULL)
	{
		_appUrl = ToUTF16String(_startUrl);
		_hostUrl = ComputeHostUrl(_appUrl);
	}
	...
	DWORD exStyle = 0;
	if (_compositionHosting)
		exStyle |= WS_EX_NOREDIRECTIONBITMAP;
	else if (initParams->Transparent)
		exStyle |= WS_EX_LAYERED;

	//Create the window
	_hWnd = CreateWindowEx(
		exStyle, //WS_EX_OVERLAPPEDWINDOW, //An optional extended window style.
...
Photino::~Photino()
{
	std::vector<PhotinoSurface*> surfaces = _surfaces;
	for (auto s : surfaces)
		if (!s->IsClosed() && s->GetHwnd()) DestroyWindow(s->GetHwnd());
	_surfaces.clear();
	if (_composition != nullptr)
	{
		_composition->RevokeDropTarget(_hWnd);
		delete _composition;
		_composition = nullptr;
	}
...
{
	{
		auto found = hwndToPhotino.find(hwnd);
		Photino* self = found != hwndToPhotino.end() ? found->second : nullptr;
		if (self != nullptr)
		{
			LRESULT compositionResult = 0;
			if (self->HandleCompositionMessage(hwnd, uMsg, wParam, lParam, &compositionResult))
	...
	{
		auto found = hwndToPhotino.find(hwnd);
		if (found != hwndToPhotino.end() && found->second != nullptr)
		{
			if (wParam == FALSE)
				found->second->OnHostVisibilityChanged(false);
			else if (!IsIconic(hwnd))
				found->second->OnHostVisibilityChanged(true);
		}
		break;
	}
	case WM_CREATE:
	...
	case WM_SIZE:
	{
		...
		if (Photino)
		{
			...
			if (LOWORD(wParam) == SIZE_MAXIMIZED) {
				Photino->OnHostVisibilityChanged(true);
				Photino->InvokeMaximized();
			}
			else if (LOWORD(wParam) == SIZE_RESTORED) {
				if (IsWindowVisible(hwnd)) Photino->OnHostVisibilityChanged(true);
				Photino->InvokeRestored();
			}
			else if (LOWORD(wParam) == SIZE_MINIMIZED) {
				Photino->OnHostVisibilityChanged(false);
	...
	case WM_MOVE:
	{
		...
		if (Photino)
		{
			if (Photino->GetCompositionHosting())
...
static std::wstring ComputeHostUrl(const std::wstring& appUrl)
{
	size_t query = appUrl.find_first_of(L"?#");
	std::wstring path = query == std::wstring::npos ? appUrl : appUrl.substr(0, query);
	size_t slash = path.find_last_of(L'/');
	if (slash == std::wstring::npos || slash <= 7) return std::wstring();
	return path.substr(0, slash + 1) + L"__photino_host__.html";
}

void Photino::NavigateToUrl(AutoString url)
{
	url = ToUTF16String(url);
	if (_compositionHosting && _surfaceHostMode == PHOTINO_SURFACE_HOST_FRAMES)
	{
		std::wstring hostUrl = ComputeHostUrl(url);
		if (!hostUrl.empty())
		{
			_appUrl = url;
			_hostUrl = hostUrl;
			_webviewWindow->Navigate(_hostUrl.c_str());
			return;
		}
		_hostUrl.clear();
	}
...
void Photino::SetBackgroundColor(int r, int g, int b, int a)
{
	...
	if (SUCCEEDED(_webviewController->QueryInterface(IID_PPV_ARGS(&controller2))))
		controller2->put_DefaultBackgroundColor(_backgroundColor);
}
		...
		Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>(
			[this](HRESULT result, ICoreWebView2Environment* env) -> HRESULT {
				...
				if (_compositionHosting)
				{
					...
					{
						_composition = new PhotinoCompositionHost(this, _hWnd);
						if (_composition->Initialize())
						{
							env3->CreateCoreWebView2CompositionController(_hWnd, Callback<ICoreWebView2CreateCoreWebView2CompositionControllerCompletedHandler>(
								[this](HRESULT result, ICoreWebView2CompositionController* compositionController) -> HRESULT {
									...
									HRESULT queryResult = compositionController->QueryInterface(IID_PPV_ARGS(&controller));
									if (queryResult != S_OK) { return queryResult; }
									_compositionController->put_RootVisualTarget(_composition->GetRootVisualTarget());
									OnControllerCreated(controller.get());
									return S_OK;
								}).Get());
							return S_OK;
						}
...
void Photino::OnControllerCreated(ICoreWebView2Controller* controller)
{
	HRESULT envResult = controller->QueryInterface(&_webviewController);
	if (envResult != S_OK) { return; }
	...
	std::wstring bridgeScript = BuildBridgeScript();
	_webviewWindow->AddScriptToExecuteOnDocumentCreated(bridgeScript.c_str(), nullptr);
	...
	_webviewWindow->add_WebResourceRequested(Callback<ICoreWebView2WebResourceRequestedEventHandler>(
		[this](ICoreWebView2* sender, ICoreWebView2WebResourceRequestedEventArgs* args)
		{
			...
			if (TryServeHostPage(uriString, args))
				return S_OK;
	...
	if (_compositionHosting && _compositionController)
	{
		...
		_compositionController->add_CursorChanged(Callback<ICoreWebView2CursorChangedEventHandler>(
		...
		if (SUCCEEDED(_webviewController->QueryInterface(IID_PPV_ARGS(&controller3))))
		{
			...
			controller3->add_RasterizationScaleChanged(Callback<ICoreWebView2RasterizationScaleChangedEventHandler>(
				[this](ICoreWebView2Controller* sender, IUnknown* args) -> HRESULT {
					UpdateWebViewLayout();
					return S_OK;
				}).Get(), &scaleToken);
		...
		_webviewWindow->add_ContentLoading(Callback<ICoreWebView2ContentLoadingEventHandler>(
			[this](ICoreWebView2* sender, ICoreWebView2ContentLoadingEventArgs* args) -> HRESULT {
				UpdateWebViewLayout();
				return S_OK;
			}).Get(), &contentLoadingToken);

		_composition->RegisterDropTarget(_hWnd, nullptr);

		wil::com_ptr<ICoreWebView2_4> webview4;
		if (UsesFrameHost() && SUCCEEDED(_webviewWindow->QueryInterface(IID_PPV_ARGS(&webview4))))
		{
			...
			webview4->add_FrameCreated(Callback<ICoreWebView2FrameCreatedEventHandler>(
				[this](ICoreWebView2* sender, ICoreWebView2FrameCreatedEventArgs* args) -> HRESULT {
					...
					if (FAILED(frame->QueryInterface(IID_PPV_ARGS(&frame2)))) return S_OK;
					EventRegistrationToken frameMessageToken;
					frame2->add_WebMessageReceived(Callback<ICoreWebView2FrameWebMessageReceivedEventHandler>(
						[this](ICoreWebView2Frame* frameSender, ICoreWebView2WebMessageReceivedEventArgs* messageArgs) -> HRESULT {
							wil::unique_cotaskmem_string message;
							messageArgs->TryGetWebMessageAsString(&message);
							if (message) _webMessageReceivedCallback(message.get());
							return S_OK;
						}).Get(), &frameMessageToken);
					return S_OK;
				}).Get(), &frameCreatedToken);
		}

		UINT32 browserProcessId = 0;
		if (SUCCEEDED(_webviewWindow->get_BrowserProcessId(&browserProcessId)))
			_composition->EnsurePopupHook(browserProcessId);
		UpdateWebViewLayout();
	}

	if (UsesFrameHost())
		_webviewWindow->Navigate(_hostUrl.c_str());
	else if (_startUrl != NULL)
	...
	else if (_hasBackgroundColor)
		SetBackgroundColor(_backgroundColor.R, _backgroundColor.G, _backgroundColor.B, _backgroundColor.A);
	...
	if (_pendingSuspendMask != 0)
	{
		unsigned int pending = _pendingSuspendMask;
		_pendingSuspendMask = 0;
		SuspendResources(pending);
	}
	if (_autoSuspendMask != 0 && (IsIconic(_hWnd) || !IsWindowVisible(_hWnd)))
		OnHostVisibilityChanged(false);
}

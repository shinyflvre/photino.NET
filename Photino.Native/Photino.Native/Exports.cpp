...
extern "C"
{
	...
	EXPORTED void Photino_SuspendResources(Photino* instance, unsigned int mask)
	{
		instance->SuspendResources(mask);
	}

	EXPORTED void Photino_ResumeResources(Photino* instance, unsigned int mask)
	{
		instance->ResumeResources(mask);
	}

	EXPORTED unsigned int Photino_GetSuspendedResources(Photino* instance)
	{
		return instance->GetSuspendedResources();
	}

	EXPORTED void Photino_SetAutoSuspendOnMinimize(Photino* instance, unsigned int mask)
	{
		instance->SetAutoSuspendOnMinimize(mask);
	}

	EXPORTED unsigned int Photino_GetAutoSuspendOnMinimize(Photino* instance)
	{
		return instance->GetAutoSuspendOnMinimize();
	}

	EXPORTED void Photino_GetCompositionHosting(Photino* instance, bool* enabled)
	{
		*enabled = instance->GetCompositionHosting();
	}

	EXPORTED void Photino_SetBackgroundColor(Photino* instance, int r, int g, int b, int a)
	{
		instance->SetBackgroundColor(r, g, b, a);
	}

	EXPORTED PhotinoSurface* Photino_CreateSurface(Photino* instance, PhotinoSurfaceParams* params)
	{
		return instance->CreateSurface(params);
	}

	EXPORTED int Photino_Surface_GetId(PhotinoSurface* surface)
	{
		return surface->GetId();
	}

	EXPORTED HWND Photino_Surface_GetHwnd(PhotinoSurface* surface)
	{
		return surface->GetHwnd();
	}

	EXPORTED void Photino_Surface_Close(PhotinoSurface* surface)
	{
		surface->Close();
	}

	EXPORTED void Photino_Surface_SetRegion(PhotinoSurface* surface, int x, int y, int width, int height)
	{
		surface->SetRegion(x, y, width, height);
	}

	EXPORTED void Photino_Surface_GetRegion(PhotinoSurface* surface, int* x, int* y, int* width, int* height)
	{
		if (x) *x = surface->RegionX();
		if (y) *y = surface->RegionY();
		if (width) *width = surface->RegionWidth();
		if (height) *height = surface->RegionHeight();
	}

	EXPORTED void Photino_Surface_SetAutoLayout(PhotinoSurface* surface, bool autoLayout)
	{
		surface->SetAutoLayout(autoLayout);
	}

	EXPORTED void Photino_Surface_GetAutoLayout(PhotinoSurface* surface, bool* autoLayout)
	{
		*autoLayout = surface->IsAutoLayout();
	}

	EXPORTED void Photino_Surface_SetVisible(PhotinoSurface* surface, bool visible)
	{
		surface->SetVisible(visible);
	}

	EXPORTED void Photino_Surface_GetVisible(PhotinoSurface* surface, bool* visible)
	{
		*visible = surface->IsVisible();
	}

	EXPORTED void Photino_Surface_SetPosition(PhotinoSurface* surface, int x, int y)
	{
		surface->SetPosition(x, y);
	}

	EXPORTED void Photino_Surface_GetPosition(PhotinoSurface* surface, int* x, int* y)
	{
		surface->GetPosition(x, y);
	}

	EXPORTED void Photino_Surface_SetSize(PhotinoSurface* surface, int width, int height)
	{
		surface->SetSize(width, height);
	}

	EXPORTED void Photino_Surface_GetSize(PhotinoSurface* surface, int* width, int* height)
	{
		surface->GetSize(width, height);
	}

	EXPORTED void Photino_Surface_SetMinSize(PhotinoSurface* surface, int width, int height)
	{
		surface->SetMinSize(width, height);
	}

	EXPORTED void Photino_Surface_SetTitle(PhotinoSurface* surface, AutoString title)
	{
		surface->SetTitle(title);
	}

	EXPORTED void Photino_Surface_SetResizable(PhotinoSurface* surface, bool resizable)
	{
		surface->SetResizable(resizable);
	}

	EXPORTED void Photino_Surface_SetTopmost(PhotinoSurface* surface, bool topmost)
	{
		surface->SetTopmost(topmost);
	}

	EXPORTED void Photino_Surface_Center(PhotinoSurface* surface)
	{
		surface->Center();
	}

	EXPORTED void Photino_Surface_Activate(PhotinoSurface* surface)
	{
		surface->Activate();
	}

	EXPORTED double Photino_Surface_GetDpiScale(PhotinoSurface* surface)
	{
	...
	EXPORTED void Photino_SuspendResources(Photino* instance, unsigned int mask) {}
	EXPORTED void Photino_ResumeResources(Photino* instance, unsigned int mask) {}
	EXPORTED unsigned int Photino_GetSuspendedResources(Photino* instance) { return 0; }
	EXPORTED void Photino_SetAutoSuspendOnMinimize(Photino* instance, unsigned int mask) {}
	EXPORTED unsigned int Photino_GetAutoSuspendOnMinimize(Photino* instance) { return 0; }
	EXPORTED void Photino_GetCompositionHosting(Photino* instance, bool* enabled) { *enabled = false; }
	EXPORTED void Photino_SetBackgroundColor(Photino* instance, int r, int g, int b, int a) {}
	EXPORTED void* Photino_CreateSurface(Photino* instance, void* params) { return nullptr; }
#endif
#ifdef __APPLE__
